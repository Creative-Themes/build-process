  ...
  options.entries.map(entry => {
    if (entry.forEachFolderIn) {
      ...
      folders.map(folder => {
        ...
        toPush.context = path.join(process.cwd(), entry.forEachFolderIn, folder)
        ...
        if (
          fs.existsSync(path.join(entry.forEachFolderIn, folder, entry.entry))
        ) {
          ...
          rules: [
            {
              ...
              options: {
                ...
                plugins: [
                  ...
                  .concat(
                    isGettextMode
                      ? [require.resolve('../../lib/i18n-babel-plugin.js')]
                      : []
                  )
