...
function run(done) {
	...
	spawn(bin, {stdio: 'inherit'}).on('exit', function() {
		done()
	})
}
...
function updateFlowTyped() {
	...
	if (!shelljs.which('flow-typed')) {
		logger.error(
			'We tried to install flow-typed for you buy without success. Please install it by hand.'
		)
		shelljs.exit(1)
	}
...
function maybeAddFlowDirsToGitIgnore() {
	maybeAddToGitIgnore(['ct-flow-typed/', 'flow-typed/'])
}
